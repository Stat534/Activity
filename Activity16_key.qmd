...
GGHB.IZ |> 
  ...
  scale_fill_viridis_b() +
  ggtitle('SMR for Respiratory Hospital Admissions')
```
...
```{r}
W_list <- nb2listw(poly2nb(respiratory_admissions), style = 'B')
W_mat <- nb2mat(poly2nb(respiratory_admissions), style = 'B')

moran.test(respiratory_admissions$observed, W_list, alternative = 'two.sided')
geary.test(respiratory_admissions$SMR, W_list, alternative = 'two.sided')
```
...
chain1 <- S.CARdissimilarity(formula=formula, data=respiratory_admissions, 
...
respiratory_admissions$risk <- chain1$fitted.values / 
